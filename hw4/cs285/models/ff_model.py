...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        # obs_normalized = # TO DO(Q1)
        # acs_normalized = # TO DO(Q1)
        ...
        # TO DO(Q1) compute delta_pred_normalized and next_obs_pred
        # Hint: as described in the PDF, the output of the network is the
        # *normalized change* in state, i.e. normalized(s_t+1 - s_t).
        # delta_pred_normalized = # TO DO(Q1)
        # next_obs_pred = # TO DO(Q1)
        delta_pred_normalized = self.delta_network(concatenated_input)
        next_obs_pred =  unnormalize(delta_pred_normalized, 
                                    ptu.from_numpy(delta_mean), 
                                    ptu.from_numpy(delta_std))  + ptu.from_numpy(obs_unnormalized)
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        """
        # prediction = # TO DO(Q1) get numpy array of the predicted next-states (s_t+1)
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
        prediction = ptu.to_numpy(self(obs, acs, **data_statistics)[0])
        return prediction
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        :return:
        """
        # target = # TO DO(Q1) compute the normalized target for the model.
        ...
        # loss = # T ODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
