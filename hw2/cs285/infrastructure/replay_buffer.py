    ...
    def add_rollouts(self, paths, noised=False):
        ...
        if self.obs is None:
            ...
            if isinstance(unconcatenated_rews, list):
                self.unconcatenated_rews += unconcatenated_rews  # TO DO keep only latest max_size around
            else:
                self.unconcatenated_rews.append(unconcatenated_rews)  # TO DO keep only latest max_size around
